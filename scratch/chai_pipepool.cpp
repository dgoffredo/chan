...
void makePipe(int (&files)[2]) {
    if (pipe(files)) {
        throw error(errno,
                    "Unable to allocate a new pipe using POSIX pipe() "
                    "in chai::PipePool::take()");
    }
}
...
void drain(int file) {
    ...
    if (flags == -1) {
        throw error(errno,
                    "Unable to get pipe flags in drain() in "
                    "chai::PipePool::giveBack");
    }
        ...
        if (rcode == 0) {
            ...
            if (errorCode == EAGAIN) {
                ...
                throw error(errorCode,
                            "Unable to read remaining data from "
                            "pipe buffer in chai::PipePool::giveBack");
            }
        }
    ...
    if (fcntl(file, F_SETFL, flags) == -1) {
        throw error(errno,
                    "Unable restor pipe's flags in drain() in "
                    "chai::PipePool::giveBack");
    }
}
